...
<body>
  ...
  <script>
    // Helper toLower like C++ toLower function
    function toLower(s){ return s.toLowerCase(); }

    // Question structure recreated exactly from the C++ source (prompts and answers copied verbatim)
    const quiz = [
        // ------------------------- SATIRE QUESTIONS -------------------------
        ...
        // ------------------------- CROSSWORD VOCAB (ALREADY INCLUDED BUT MERGED) -------------------------
    ...
    // Copy of original total and counters
    const TOTAL = quiz.length;
    let correctCount = 0;

    // We'll make a mutable array copy to remove items as answered, like the C++ version
    let pool = quiz.slice();

    // Console area to mimic cout output
    ...
    // Initial messages that match the C++ program's output
    // The C++ printed: "lock tf in\n" and then "btw not case sensitive" << total << "\n"
    ...
    function nextQuestion(){
      ...
      // store current question index in a special field
    ...
    // Skip to show answer (similar to answering incorrectly and leaving it in C++ code they erase only when correct, so here we will reveal answer and remove it to progress)
    skipBtn.addEventListener('click', ()=>{
      if(!current) return;
      appendConsole('incorrect. correct answer was ' + current.item.answer + '\n');
      // remove from pool to match C++ behavior where incorrect does not erase — but user requested EXACT copy; however C++ does NOT remove on incorrect, so to truly match, we should NOT remove on skip.
      // To mirror C++ exactly: only remove on correct. So DO NOT remove here. Instead just move to next random question.
      nextQuestion();
    });
